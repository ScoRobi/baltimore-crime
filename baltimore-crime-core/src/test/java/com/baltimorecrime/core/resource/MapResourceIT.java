...
public class MapResourceIT {
  ...
  public void requestDataPostWithParams() {
    given().contentType("application/json").with().body("{\n"
        ...
        + "}")
        .expect().statusCode(200).body(equalTo("[{\"crimeDate\":\"2016-06-18\",\"crimeTime\":\"00:33:00\",\"crimeCode\":\"4E\",\"location\":\"2700 CHESLEY AVE\",\"weapon\":\"HANDS\",\"post\":424,\"district\":\"NORTHEASTERN\",\"neighborhood\":\"North Harford Road\",\"longitude\":-76.55559,\"latitude\":39.3679}]"))
        .when().post("/core/map/data");
  }
  ...
  public void requestDataPostInvalidParam() {
    given().contentType("application/json").with().body("{\n"
        + "  \"weapons\":[\"INVALID\"]\n"
        + "}")
        .expect().statusCode(200).body(equalTo("[]")).when().post("/core/map/data");
  }

  @Test
  public void testReadDataRanges() {
    when().get("/core/map/data/bounds").then().statusCode(200).body(
  ...
  public void testPing() {
    when().get("/core/map").then().statusCode(200).body(equalTo("Ready for service!"));
  }
}
